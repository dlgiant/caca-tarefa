...
class DatabaseBackup {
  ...
  private async uploadToS3(filePath: string, fileName: string): Promise<void> {
    ...
    try {
      ...
      console.log(`✅ Upload concluído: s3://${this.config.s3Bucket}/database-backups/${fileName}`);
  ...
  async cleanupOldBackups(): Promise<void> {
    console.log(`🧹 Limpando backups antigos (mantendo ${this.config.maxBackups} mais recentes)...`);
