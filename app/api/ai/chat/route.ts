    ...
    switch (commandResult.action) {
        ...
        if (commandResult.parameters.title) {
          taskData = await prisma.task.create({
            data: {
              ...
              priority: commandResult.parameters.priority || 'medium',
              status: 'todo',
          ...
          response = 'Por favor, forneça um título para a tarefa. Por exemplo: "Criar tarefa Revisar documento"';
        }
        break;
      ...
      case 'suggest_next':
        ...
        if (nextTask) {
          ...
          if (nextTask.dueDate) {
            const daysUntilDue = Math.ceil((new Date(nextTask.dueDate).getTime() - now.getTime()) / (1000 * 60 * 60 * 24));
            if (daysUntilDue === 0) {
              response += '\n\n⏰ **Vence hoje!**';
          ...
          response = '🎉 Parabéns! Você não tem tarefas pendentes. Que tal criar uma nova meta?';
          suggestions = ['Criar nova tarefa', 'Ver tarefas concluídas', 'Definir metas'];
        }
        break;
